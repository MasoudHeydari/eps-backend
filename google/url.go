...
// BuildURL builds Google query URL from Query struct
func BuildURL(q core.Query) (string, error) {
	...
	if q.Offset != 0 {
		params.Add("start", strconv.Itoa(q.Offset))
	}

	// Set Location parameter
	if q.Location != "" {
		params.Add("cr", "country"+strings.ToUpper(q.Location))
	}
	...
	params.Add("nfpr", "1") // Do not autocorrect search queries
